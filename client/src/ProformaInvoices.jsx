...
const ProformaInvoices = () => {
  ...
  const filteredRows = useMemo(() => invoices.filter((invoice) => {
  ...
  const handleDelete = async (invoiceId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/proforma-invoices/${invoiceId}`, { method: "DELETE", headers: authHeaders });
  ...
  const csvRows = useMemo(() => filteredRows.map((i) => ({ invoice: i.invoiceNumber || "-", customer: i.customerName, status: i.status, total: i.grandTotal, issueDate: i.issueDate })), [filteredRows]);

  return (
    <div className="comman-page container-fluid py-4">
      <PageSectionHeader
        title="Proforma Invoices"
        subtitle="Proforma invoices for dealers"
        breadcrumbItems={[{ label: "Home", to: "/dashboard" }, { label: "Proforma" }]}
        actions={(
          <>
            <button className="btn btn-outline-success" onClick={() => exportRowsToCsv(csvRows, "proforma.csv")}>Export</button>
      ...
      <div className="card border-0 shadow-sm mb-3">
        <div className="card-body">
          <div className="row g-3">
            ...
            <div className="col-12 col-md-2"><select className="form-select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">All Status</option><option value="Draft">Draft</option><option value="Issued">Issued</option><option value="Paid">Paid</option><option value="Cancelled">Cancelled</option></select></div>
                ...
                {loading ? (
                  ...
                  pagedItems.map((invoice) => (
                    <tr key={invoice._id}>
                      ...
                      <td><strong>{invoice.customerName}</strong>{invoice.customerMobile && <div className="text-muted small">{invoice.customerMobile}</div>}</td>
